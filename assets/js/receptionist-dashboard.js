...
(function($) {
    ...
    if (typeof clinicaReceptionistAjax === 'undefined') {
        console.warn('clinicaReceptionistAjax nu este disponibil, folosesc date demo');
        window.clinicaReceptionistAjax = {
            ajaxurl: '/wp-admin/admin-ajax.php',
            nonce: 'demo_nonce'
        };
    } else {
        console.log('clinicaReceptionistAjax este disponibil:', clinicaReceptionistAjax);
    }
    ...
    $(document).ready(function() {
        const dashboard = $('.clinica-receptionist-dashboard');
        ...
        function initTabs() {
            ...
            $('.clinica-receptionist-tab-button').on('click', function() {
        ...
        function activateTab(tab) {
            ...
            localStorage.setItem('clinica_receptionist_active_tab', tab);
            
            // Activează tab-ul
            $('.clinica-receptionist-tab-button').removeClass('active');
            $(`.clinica-receptionist-tab-button[data-tab="${tab}"]`).addClass('active');
            
            // Afișează conținutul
            $('.clinica-receptionist-tab-content').removeClass('active');
            $(`.clinica-receptionist-tab-content[data-tab="${tab}"]`).addClass('active');
        ...
        function initActions() {
            ...
            $('[data-action="view-calendar"]').on('click', function() {
                $('.clinica-receptionist-tab-button[data-tab="calendar"]').click();
            });
        ...
        const action = clinicaReceptionistAjax.actions ? clinicaReceptionistAjax.actions.overview : 'clinica_receptionist_dashboard_overview';
        $.post(clinicaReceptionistAjax.ajaxurl, {
            action: action,
        ...
        function loadTabData(tab) {
            const content = $(`.clinica-receptionist-tab-content[data-tab="${tab}"]`);
            
            if (content.find('.clinica-receptionist-loading').length > 0) {
                switch(tab) {
                    case 'overview':
                        loadOverviewData();
                        break;
        ...
        function loadAppointments() {
            const content = $('.clinica-receptionist-tab-content[data-tab="appointments"]');
            const action = clinicaReceptionistAjax.actions ? clinicaReceptionistAjax.actions.appointments : 'clinica_receptionist_dashboard_appointments';
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        const tbody = $('#receptionist-appointments-tbody');
                        if (tbody.length) {
                            // Extrage doar rândurile din tabelul de programări (acum sunt div-uri)
                            const appointmentsTable = $(response.data.html).find('.clinica-receptionist-appointments-table');
                            if (appointmentsTable.length) {
                                // Extrage doar rândurile de programări (fără header)
                                const appointmentItems = appointmentsTable.find('.clinica-receptionist-appointment-item');
        ...
        function loadPatients() {
            const content = $('.clinica-receptionist-tab-content[data-tab="patients"]');
            
            // Afișează interfața de căutare și filtrare
            const patientsHtml = `
                <div class="clinica-receptionist-patients-container">
        ...
        function loadCalendar() {
            const content = $('.clinica-receptionist-tab-content[data-tab="calendar"]');
            
            $.ajax({
                url: clinicaReceptionistAjax.ajaxurl,
                type: 'POST',
                data: {
                    action: clinicaReceptionistAjax.actions ? clinicaReceptionistAjax.actions.calendar : 'clinica_receptionist_dashboard_calendar',
                    nonce: clinicaReceptionistAjax.nonce
                },
                success: function(response) {
                    if (response.success) {
                        ...
                        const html = `
                            <div class="clinica-receptionist-calendar">
        ...
        function loadReports() {
            const content = $('.clinica-receptionist-tab-content[data-tab="reports"]');
            
            $.ajax({
                url: clinicaReceptionistAjax.ajaxurl,
                type: 'POST',
                data: {
                    action: clinicaReceptionistAjax.actions ? clinicaReceptionistAjax.actions.reports : 'clinica_receptionist_dashboard_reports',
                    nonce: clinicaReceptionistAjax.nonce
                },
                success: function(response) {
                    if (response.success) {
                        const data = response.data;
                        const html = `
                            <div class="clinica-receptionist-reports">
                                <h3>Rapoarte</h3>
        ...
        function updateStats(stats) {
            $('.clinica-receptionist-stat-card').each(function() {
                const card = $(this);
        ...
        $(document).on('keydown', function(e) {
            if (e.ctrlKey || e.metaKey) {
                switch(e.key) {
                    case '1':
                        e.preventDefault();
                        $('.clinica-receptionist-tab-button[data-tab="overview"]').click();
                        break;
                    case '2':
                        e.preventDefault();
                        $('.clinica-receptionist-tab-button[data-tab="appointments"]').click();
                        break;
                    case '3':
                        e.preventDefault();
                        $('.clinica-receptionist-tab-button[data-tab="patients"]').click();
                        break;
                    case '4':
                        e.preventDefault();
                        $('.clinica-receptionist-tab-button[data-tab="calendar"]').click();
                    ...
                    case '5':
                        e.preventDefault();
                        $('.clinica-receptionist-tab-button[data-tab="reports"]').click();
                        break;
                }
            }
        });
        ...
        setInterval(function() {
            const activeTab = $('.clinica-receptionist-tab-button.active').data('tab');
            if (activeTab) {
        ...
        window.loadPatientsData = function() {
            console.log('Încărcare pacienți cu nonce:', clinicaReceptionistAjax.nonce);
            ...
            const ajaxData = {
                action: clinicaReceptionistAjax.actions ? clinicaReceptionistAjax.actions.patients : 'clinica_receptionist_dashboard_patients',
                nonce: clinicaReceptionistAjax.nonce,
        ...
        function loadAppointmentModalData(patientId) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'clinica_receptionist_get_appointment_modal_data',
                    nonce: clinicaReceptionistAjax.nonce
                },
                success: function(response) {
                    hideLoadingModal();
                        ...
                        <div class="clinica-modal-body">
                            <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="af-created-by" readonly value="${data.current_user_name || 'Receptionist'}" style="background-color: #f8f9fa; cursor: not-allowed;">
                                </div>
        ...
        function loadPatientData(patientId) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'clinica_receptionist_get_patient_data',
    ...
    window.savePatientChanges = function() {
        const formData = {
            action: 'clinica_receptionist_update_patient',
            nonce: clinicaReceptionistAjax.nonce,
        ...
        window.saveNewAppointment = function() {
            const formData = {
                action: 'clinica_receptionist_create_appointment',
                nonce: clinicaReceptionistAjax.nonce,
        ...
        function searchPatientsSuggestions(searchTerm, inputId) {
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'clinica_receptionist_search_patients_suggestions',
                    nonce: clinicaReceptionistAjax.nonce,
        ...
        function searchFamiliesSuggestions(searchTerm, inputId) {
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'clinica_receptionist_search_families_suggestions',
                    nonce: clinicaReceptionistAjax.nonce,
                    search_term: searchTerm
                },
                success: function(response) {
        ...
        console.log('Dashboard Receptionist inițializat cu succes');
    });
    ...
    window.saveNewAppointmentAdvanced = function() {
        ...
        const formData = {
            action: 'clinica_receptionist_create_appointment_advanced',
            nonce: clinicaReceptionistAjax.nonce,
            patient_id: patientId,
            service_id: serviceId,
            doctor_id: doctorId,
            ...
            status: status,
            created_by_type: 'receptionist', // Întotdeauna 'receptionist' pentru dashboard-ul receptionistului
            notes: notes,
