...
echo "=== ACTUALIZARE VALIDARE TELEFOANE CU FORMATE ROMÂNEȘTI EXTINSE (FUNCȚIONAL) ===\n\n";
...
function validatePhoneWithFormats($phone) {
    ...
    // Format românesc cu separatori: 07xx.xxx.xxx sau 07xx-xxx-xxx
    if (preg_match('/^(\+40|0)[0-9]{2}[.\-][0-9]{3}[.\-][0-9]{3}$/', $phone)) {
        return true; // Format românesc cu separatori
    }
...
function getPhoneErrorType($phone) {
    ...
    if (preg_match('/^(\+40|0)/', $phone)) {
        // Verifică dacă are formatul corect cu separatori
        if (preg_match('/^(\+40|0)[0-9]{2}[.\-][0-9]{3}[.\-][0-9]{3}$/', $phone)) {
            return 'VALID ROMÂNIA CU SEPARATORI';
        }
