<?php
/**
 * Script pentru corectarea numelor familiilor conform convenției românești
 * În română: NUMELE DE FAMILIE ESTE PRIMUL, nu ultimul!
 */

echo "<h1>Corectare Nume Familii - Convenție Românească</h1>";
...
echo "<h2>2. Corectare nume familii cu numele de familie reale (convenție românească):</h2>";
            ...
            if (!empty($head['display_name'])) {
                ...
                if (count($name_parts) > 1) {
                    ...
                    if (!empty($first_name_clean)) {
                        $new_family_name = ucfirst(strtolower($first_name_clean));
                    }
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        echo "<h3>Primele 10 nume de familie (corectate):</h3>";
...
echo "<li>\"Danciu Adelina-Claudia\" → Numele de familie: <strong>Danciu</strong></li>";
echo "</ul>";
