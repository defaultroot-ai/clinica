...
jQuery(document).ready(function($) {
                ...
                resp.data.forEach(function(doctor) {
                    ...
                    if (parseInt(doctor.id) !== parseInt(transferData.doctorId)) {
                        ...
                        btn.on('click', function() {
                            ...
                            // Încarcă calendarul pentru noul doctor
                            loadTransferAvailableDays(doctor.id, transferData.serviceId);
                            
                            // Dacă există o dată selectată, încarcă sloturile
                            if (transferData.date) {
                                loadTransferSlots(doctor.id);
                            } else {
                                // Dacă nu există dată, resetează sloturile
                                $('#transfer-slots').html('<div class="slot-btn disabled">Selectează o dată</div>');
    ...
    // Când se schimbă data, actualizează sloturile pentru doctorul selectat
    $('#transfer-date-picker').on('change', function() {
        var newDate = $(this).val();
        if (newDate) {
            transferData.date = newDate;
            // Resetează doar sloturile, nu doctorul
            $('#transfer-slots').html('');
            transferData.selectedSlot = '';
            $('#transfer-modal-confirm').prop('disabled', true);
            
            // Dacă există un doctor selectat, încarcă sloturile pentru noua dată
            if (transferData.selectedDoctorId) {
                loadTransferSlots(transferData.selectedDoctorId);
            }
        }
    });
    ...
    function loadTransferSlots(doctorId) {
        if (!doctorId || !transferData.date) {
            $('#transfer-slots').html('<div class="slot-btn disabled">Selectează un doctor și o dată</div>');
            return;
        }
